...
let btns = document.querySelectorAll(".quick_view_btn")
btns.forEach(btn => {
...
async function quickView(product_handle) {
    ...
    let variantSelect = quickViewModal.querySelector(".product_options select[name='id']")
    let compareAtPrice = ""
    ...
    if (product.compare_at_price != product.price && product.compare_at_price > 0) {
        ...
        compareAtPrice.innerText = quickMoneyFormat(product.compare_at_price, format)
    ...
    if (product.available) {
        ...
        buy.querySelector(".text").textContent = "Buy Now"
        buy.disabled = false;
        add.disabled = false;
    } else {
        add.querySelector(".text").textContent = "Out of Stock"
        add.disabled = true;
        buy.querySelector(".text").textContent = "Out of Stock"
        buy.disabled = true;
    }
    ...
    product.variants.forEach(variant => {
        ...
        option.innerHTML = `${variant.title} - ${quickMoneyFormat(variant.price, format)}`
    ...
    product.options.forEach(option => {
        ...
        option.values.forEach(value => {
            ...
            listItem.innerHTML = `<div class="${option.name == "Color" ? "color_input radio_input" : "radio_input"}"><input type="radio" value="${value}" name="${option.name}" /><label><span class="val">${value}</span></label></div>`
    ...
    document.querySelectorAll("#quickview_options input[type='radio']").forEach(radio => {
        radio.addEventListener('change', () => {

            let selectedOptions = [];

            document.querySelectorAll("#quickview_options input[type='radio']:checked").forEach(radio => {
            ...
            radio.closest("fieldset").querySelector(".option_val").innerText = radio.value
            if (matchedVariant) {
                document.querySelector("#quickview_product_id").value = matchedVariant.id;

                document.querySelector("#quickview_variant_select").value = matchedVariant.id;
            }
            ...
            if (matchedVariant) {
                ...
                if (matchedVariant.compare_at_price) {
                    compareAtPrice.innerText = formatMoney(matchedVariant.compare_at_price, format);
                }
            }
    ...
    document.querySelector("#quickView .product-buy").addEventListener("click", (e) => {
        e.preventDefault();
...
window.addEventListener("keyup", (e) => {
    let quickViewModal = document.querySelector("#quickView")
    if (e.key == "Escape" && quickViewModal.classList.contains("quickview_active")) {
        quickViewModal.classList.remove("quickview_active")
    }
})
